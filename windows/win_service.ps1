...
If ($state -eq 'create' -and [string]::IsNullOrEmpty($path))
{
    Fail-Json $result "start mode is 'create', but path is empty! path must not be null."
}
...
If (-and $svcName -ne $svc.ServiceName) {
